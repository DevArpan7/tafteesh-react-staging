...
const ShelterHomeDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { shelterHomeList, onSelectRow, survivorName ,isLoading} = props;
  ...
  const customerService =
    shelterHomeList && shelterHomeList.length > 0 && shelterHomeList;
  ...
  const shelterhomeBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.shelter_home}`}>
        {rowData.shelter_home}
      </span>
    );
  };
  ...
  return (
    <div className="dataTableFilter">
        ...
        >
          ...
          <Column
            header="Shelter Home"
            filterField="shelter_home"
            // dataType="date"
            style={{ minWidth: "10rem" }}
            body={shelterhomeBodyTemplate}
            filter
...
export default ShelterHomeDataTable;
