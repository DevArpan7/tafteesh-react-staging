...
const OrganizationsDataTable = (props) => {
  ...
  const { organizationList, onSelectRow,isLoading } = props;
  ...
  const customerService =
    organizationList && organizationList.length > 0 && organizationList;
  ...
  const emailCountBodyTemplate = (rowData) => {    
    return (
  ...
  const cityBodyTemplate = (rowData) => {    
    return (
      <span className={`customer-badge status-${rowData.city}`}>
        {rowData.city}
      </span>
    );
  };
  ...
  const stateBodyTemplate = (rowData) => {    
    return (
      <span className={`customer-badge status-${rowData.state}`}>
        {rowData.state}
      </span>
    );
  };
