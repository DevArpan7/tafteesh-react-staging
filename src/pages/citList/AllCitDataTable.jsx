...
const AllCitDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { allCitList, onSelectRow,isLoading,handleChange } = props;
  ...
  const customerService =
    allCitList && allCitList.length > 0 && allCitList;
  ...
  const scoreBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.overall_score ? rowData.overall_score:0}`}>
        {rowData.overall_score ? rowData.overall_score : 0}
      </span>
    );
  };
  ...
  const statusMarkBodyTemplate = rowData => {
    ...
    return (
      <span className={`customer-badge status-${rowData.approval === true && "Approved"}`}>
   ...
   <FormControl component="fieldset" variant="standard">
      <FormGroup>
        <FormControlLabel
          ...
          label={rowData.approval === true ? "Approved" : "Pending"}
        />
  ...
  const dateBodyTemplate = (rowData) => {
    return formatDate(rowData.assessment_date && rowData.assessment_date);
  };
...
export default AllCitDataTable;