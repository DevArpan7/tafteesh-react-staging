...
const DataTablePcFilter = (props) => {
  ...
  const { survivorPcList, onSelectRow, survivorName,isLoading } = props;
  ...
  const dateOfTraffickingBodyTemplate = (rowData) => {
    return formatDate(rowData.started_date);
  };

  const dgenderBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.court.name}`}>
        {rowData.court.name}
      </span>
    );
  };
  ...
  const phoneBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.current_status.name.toUpperCase()}`}>
        {rowData.current_status.name.toUpperCase()}
      </span>
    );
  };
