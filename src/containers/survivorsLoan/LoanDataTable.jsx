...
const LoanDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { survivalLoanList, onSelectRow ,isLoading,selectedData} = props;
  ...
  useEffect(() => {
    ...
    let arr = [];
    survivalLoanList &&
      survivalLoanList.length > 0 &&
      survivalLoanList.map((item) => {
  ...
  const customerService =
    survivalLoanList && survivalLoanList.length > 0 && survivalLoanList;
  ...
  const total_paid_amountBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.total_paid_amount}`}>
        {rowData.total_paid_amount}
      </span>
    );
  };
  ...
  const amountBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.amount}`}>
        {rowData.amount}
      </span>
    );
  };
  ...
  const whereBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.where}`}>
        {rowData.where}
      </span>
    );
  };
    ...
    <div className="dataTableFilter">
        ...
        >
          ...
          />
            <Column
            header="Repayment"
            filterField="repayment_per_month"
            // dataType="date"
            style={{ minWidth: "10rem" }}
          ...
          />
            <Column
            header="Paid Amount"
            filterField="total_paid_amount"
            style={{ minWidth: "10rem" }}
            body={total_paid_amountBodyTemplate}
            filter
            filterElement={dateFilterTemplate}
          />
             <Column
