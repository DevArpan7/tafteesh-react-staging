...
const PoliceStationsDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { policeStationList, onSelectRow,isLoading } = props;
  ...
  const customerService =
    policeStationList && policeStationList.length > 0 && policeStationList;
  ...
  const distBodyTemplate = (rowData) => {
    // return formatDate(rowData.received_on);
    
    return (
      <span className={`customer-badge status-${rowData.districtId && rowData.districtId.name}`}>
        {rowData.districtId && rowData.districtId.name}
      </span>
    );
  };
  const blockBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.blockId && rowData.blockId.name}`}>
        {rowData.blockId && rowData.blockId.name}
      </span>
    );
  };
...
export default PoliceStationsDataTable;
