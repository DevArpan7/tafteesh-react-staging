...
const KamoOrganizations = (props) => {
  ...
  useEffect(() => {
    ...
    if (addOrgData && addOrgData.name) {
      ...
      //   setPartEmail(false);
      setFieldData({ field: "city", message: "" });
    } else if (addOrgData && addOrgData.state) {
      //   setPartEmail(false);
      setFieldData({ field: "state", message: "" });
  ...
  const addOrgFunc = (e) => {
    ...
    if (addOrgData && !addOrgData.name) {
      ...
      setFieldData({ field: "city", message: "Please enter city." });
    } else if (addOrgData && !addOrgData.state) {
      setFieldData({ field: "state", message: "Please select State." });
    } else if (addOrgData && !addOrgData.pin) {
  ...
  const exportToCsv = (e) => {
    ...
    let headers = [
      "Id,OrganizationName,PhoneNo,Email,City,State,Pin,ContactPersion",
    ];

    // Convert users data to a csv
    let usersCsv = organizationList.data.reduce((acc, user) => {
      const { _id, name, phone, email, city, state, pin, contactPersion } =
        user;
      acc.push(
        [_id, name, phone, email, city, state, pin, contactPersion].join(",")
      );
      return acc;
    }, []);
  ...
  const [sampleArr, setSampleArr] = useState([
    {
      ...
      email: "user@mail.com",
      city: "Kol",
      state: "62a9a7c3c05c6a1059f92eb0",
      pin: "123456",
              ...
              <Form>
                <Row>
                  ...
                  <Form.Group as={Col} md="6" className="mb-3">
                    ...
                    <Form.Control
                      defaultValue={
                        addOrgData && addOrgData.city && addOrgData.city
                      }
                      type="text"
                      placeholder=""
                      name="city"
                  ...
                  <Form.Group as={Col} md="6" className="mb-3">
                    ...
                    <Form.Select
                      ...
                      name="state"
                      value={addOrgData && addOrgData.state && addOrgData.state}
                    >
                      <option hidden={"true"} value="">Please select State </option>
                      {stateList &&
                        stateList.length > 0 &&
                        stateList.map((data) => {
