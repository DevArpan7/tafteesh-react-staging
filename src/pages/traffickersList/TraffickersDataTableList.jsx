...
const TraffickersDataTableList = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { traffickerList, onSelectRow,isLoading } = props;
  ...
  const addressBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.residential_address_source}`}>
        {rowData.residential_address_source}
      </span>
    );
  };
  ...
  const traffickerNameBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.trafficker_name}`}>
        {rowData.trafficker_name}
      </span>
    );
  };

  const survivorCountBodyTemplate = (rowData) => {
    // return formatDate(rowData.received_on);
    
    return (
      <span className={`customer-badge status-${rowData.survivors && rowData.survivors.length }`}>
        {rowData.survivors && rowData.survivors.length }
      </span>
    );
  };
  ...
  const isTraffickerBodyTemplate = (rowData) => {
    ...
    return (
      <span className={`customer-badge status-${rowData.survivors && rowData.survivors.length >0 ? "Yes": "No"}`}>
        {rowData.survivors && rowData.survivors.length >0 ? "Yes": "No"}
      </span>
    );
  };

  const idMarkBodyTemplate = (rowData) => {
    // return formatDate(rowData.received_on);
    
    return (
      <span className={`customer-badge status-${rowData.identification_mark}`}>
        {rowData.identification_mark}
      </span>
    );
  };
    ...
    <div className="dataTableFilter">
        ...
        >
          ...
          /> */}
            ...
            header="Name"
            // dataType="date"
            filterField="trafficker_name"
            style={{ minWidth: "16rem" }}
            body={traffickerNameBodyTemplate}
            filter
          ...
          <Column
            header="Address"
            // dataType="date"
            filterField="residential_address_source"
            style={{ minWidth: "10rem" }}
        ...
        <Column
            header={<>Survivor <label>(Count)</label></>}
            // dataType="date"
            filterField="0"
            style={{ minWidth: "8rem" }}
            body={survivorCountBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
        />
        ...
        <Column
            header="ID Mark"
            // dataType="date"
            filterField="identification_mark"
            style={{ minWidth: "12rem" }}
            body={idMarkBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
        />

        <Column
            header="Is Trafficker"
            // dataType="date"
            filterField="is_trafficker"
            style={{ minWidth: "9rem" }}
            body={isTraffickerBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
        />
        </DataTable>
...
export default TraffickersDataTableList;
