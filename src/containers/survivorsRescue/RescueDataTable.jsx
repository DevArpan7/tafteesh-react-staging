...
const RescueDataTable = (props) => {
  ...
  const { rescueList, onSelectRow,isLoading } = props;
  ...
  const sourceBodyTemplate = (rowData) => {
    return formatDate(rowData.date_of_rescue);
  ...
  const applicationNoBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.rescue_from_state && rowData.rescue_from_state.name}`}>
        {rowData.rescue_from_state && rowData.rescue_from_state.name}
      </span>
    );
  };
