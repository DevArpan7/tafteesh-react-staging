...
const NextPlanDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { nextPlanList, onSelectRow,isLoading } = props;
  ...
  const customerService =
    nextPlanList && nextPlanList.length > 0 && nextPlanList;
    ...
    return (
      <React.Fragment>
        <span className="image-text">
          {rowData.plan_for && rowData.plan_for}
        </span>
      </React.Fragment>
    );
  };
  ...
  const dateOfTraffickingBodyTemplate = (rowData) => {
    return formatDate(rowData.plan_date && rowData.plan_date);
    // return (
    //   <span className={`customer-badge status-${rowData.name_of_agency}`}>
    //     {rowData.name_of_agency}
    //   </span>
    // );
  };
  ...
  const phoneBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.to_do}`}>
        {rowData.to_do}
      </span>
    );
  };
  ...
  return (
    <div className="dataTableFilter">
      ...
      >
        ...
        <Column
          header="For"
          filterField="plan_for"
          style={{ minWidth: "12rem" }}
          body={survivorIdBodyTemplate}
          filter
          filterPlaceholder="plan_for"
...
export default NextPlanDataTable;
