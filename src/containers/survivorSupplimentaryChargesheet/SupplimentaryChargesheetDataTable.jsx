...
const SupplimentaryChargesheetDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { chargeSheetList, onSelectRow } = props;
  ...
  const chargeSheetDateBodyTemplate = (rowData) => {
  {
  ...
  const chargeSheetNoBodyTemplate = (rowData) => {
    // return formatDate(rowData.rescue_from_city);
    if(rowData.charge_sheet && rowData.charge_sheet.number)
    {return (
      <span className={`customer-badge status-${rowData.charge_sheet.number}`}>
        {rowData.charge_sheet.number}
      </span>
    );}
  };
  ...
  //   return (
  //     <span className={`customer-badge status-${rowData.fir.number}`}>
  //       {rowData.fir.number}
  //     </span>
  //   );
  // };
  ...
  const SupplimentaryChargesheetNumberBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.supplimentary_chargesheet_number}`}>
        {rowData.supplimentary_chargesheet_number}
      </span>
    );
  };
  ...
  return (
    <div className="dataTableFilter">
        ...
        <Column
            ...
            // dataType="date"
            style={{ minWidth: "12rem" }}
            body={ActBodyTemplate}
            filter
            filterElement={dateFilterTemplate}
        />
        <Column
            header="Supplimentary Charge Sheet No."
            // dataType="date"
            filterField="charge_sheet"
            style={{ minWidth: "12rem" }}
            body={SupplimentaryChargesheetNumberBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
          />
        <Column
            header="Supplimentary Charge sheet date"
            dataType="date"
            filterField="date"
            showFilterMatchModes={false}
            filterMenuStyle={{ width: "10rem" }}
            style={{ minWidth: "10rem" }}
            body={chargeSheetDateBodyTemplate}
            filter
...
export default SupplimentaryChargesheetDataTable;
