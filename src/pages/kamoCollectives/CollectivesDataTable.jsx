...
const CollectivesDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { collectivesList, onSelectRow ,isLoading} = props;
  ...
  useEffect(() => {
    ...
    let arr = [];
    collectivesList &&
      collectivesList.length > 0 &&
      collectivesList.map((item) => {
  ...
  const customerService =
    collectivesList && collectivesList.length > 0 && collectivesList;
...
export default CollectivesDataTable;