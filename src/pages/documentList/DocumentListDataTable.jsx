...
const DocumentListDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { documentList, onSelectRow,isLoading } = props;
  ...
  const customerService =
    documentList && documentList.length > 0 && documentList;
  ...
  const isReqMarkBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.is_required === true ? "Yes" : "No"}`}>
        {rowData.is_required === true ? "Yes" : "No"}
      </span>
    );
  };
 ...
 <Column
            header="Is Required"
            filterField="is_required"
            style={{ minWidth: "15rem" }}
            body={isReqMarkBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
        />
        <Column
...
export default DocumentListDataTable;
