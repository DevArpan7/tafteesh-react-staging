...
const ChargesheetDataTable = (props) => {
  const [customers1, setCustomers1] = useState([]);
  const [filters1, setFilters1] = useState(null);
  const { chargeSheetList, onSelectRow,isLoading } = props;
  ...
  const chargeSheetDateBodyTemplate = (rowData) => {
    return formatDate(rowData.charge_sheet.date);
  ...
  const locationBodyTemplate = (rowData) => {
    return (
      <span className={`customer-badge status-${rowData.location.toUpperCase()}`}>
        {rowData.location.toUpperCase()}
      </span>
    );
  };
  ...
  return (
    <div className="dataTableFilter">
        ...
        />
        <Column
            header="Charge Sheet No."
            // dataType="date"
            filterField="charge_sheet"
            style={{ minWidth: "12rem" }}
            body={chargeSheetNoBodyTemplate}
            filter
            filterElement={balanceFilterTemplate}
          />
        <Column
            header="Charge sheet date"
            dataType="date"
            filterField="charge_sheet"
...
export default ChargesheetDataTable;
